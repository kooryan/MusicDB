...
//    public static final String CONNECTION_STRING = "jdbc:sqlite:/Users/ryankoo/MusicDB/" + DB_NAME;
    ...
    public static final String CREATE_ARTIST_FOR_SONG_VIEW =
            "CREATE VIEW IF NOT EXISTS " + TABLE_ARTIST_SONG_VIEW + " AS SELECT " + TABLE_ARTISTS
                    + "." + COLUMN_ARTIST_NAME + ", " + TABLE_ALBUMS + "." + COLUMN_ALBUM_NAME + " AS " +
    ...
    // Protecting against SQL Injection -- difference is we are ending with a question mark now
...
//    public static final String QUERY_SONGS;
    ...
    public List<Artist> queryArtists(int sortOrder) { // Try with resources ensures that Statement and ResultSet are closed

        StringBuilder sb = new StringBuilder("SELECT * FROM ");
        sb.append(TABLE_ARTISTS);

        if (sortOrder != ORDER_BY_NONE) { // We could use an ENUM here too
            sb.append(" ORDER BY ");
    ...
    private int insertArtist(String name) throws SQLException {
        ...
        queryArtists.setString(1, name); // querying artist table to see if artist already exists
        ResultSet results = queryArtists.executeQuery();
        if (results.next()) {
            return results.getInt(1); // artist already exists and return id
        } else {
            // Insert the artist
            ...
            if (generatedKeys.next()) {
                return generatedKeys.getInt(1); // at position 1 because we only expect 1 key to be returned
            } else {
